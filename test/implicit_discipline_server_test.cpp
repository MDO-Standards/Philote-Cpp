...
TEST_F(ImplicitServerTest, ComputeResidualsUnlinkedPointers) {
    ...
    grpc::Status status = server_->ComputeResidualsForTesting(context_.get(), stream.get());
...
TEST_F(ImplicitServerTest, SolveResidualsUnlinkedPointers) {
    ...
    grpc::Status status = server_->SolveResidualsForTesting(context_.get(), stream.get());
...
TEST_F(ImplicitServerTest, ComputeResidualGradientsUnlinkedPointers) {
    ...
    grpc::Status status = server_->ComputeResidualGradientsForTesting(context_.get(), stream.get());
